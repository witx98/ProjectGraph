...
void graf::Graf::znajdz_funkcje_i_wagi_pomiedzy_plikami() {

	//w tej pêtli iterujemy po wszystkich plikach
	for (unsigned int i = 0; i < pliki.size(); i++) {

		//ta petla iteruje po wszystkich "poleczenia_miedzy_plikami" dla danego pliku
		for (unsigned int j = 0; j < pliki[i].polaczenia_miedzy_plikami.size(); j++) {
